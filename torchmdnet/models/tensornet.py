...
def mycompile(fn):
    return torch.compile(fn, backend="inductor", mode= "reduce-overhead")
...
torch._dynamo.disallow_in_graph(vector_to_skewtensor)
...
def decompose_tensor(tensor):
    Iij = (tensor.diagonal(offset=0, dim1=-1, dim2=-2)).mean(-1)[
        ..., None, None
    ] * torch.eye(3, 3, device=tensor.device)
...
torch._dynamo.disallow_in_graph(MyDistance.forward)
...
class TensorNetwork(nn.Module):
    def __init__(
        ...
        self.tensor_embedding = TensorNeighborEmbedding(
            hidden_channels,
            num_rbf,
            num_linears_tensor,
            num_linears_scalar,
    ...
    def forward(
        ...
        tensor_range_push("TensorNetwork")
...
@torch._dynamo.skip
def mask_edges(edge_index, edge_vec):
    mask = edge_index[0] != edge_index[1]
    edge_vec[mask] = edge_vec[mask] / torch.norm(edge_vec[mask], dim=1).unsqueeze(1)
    return edge_vec

torch._dynamo.disallow_in_graph(mask_edges)
class TensorNeighborEmbedding(MessagePassing):
    ...
    def forward(self, z, edge_index, edge_weight, edge_vec, edge_attr):
        ...
        edge_vec = mask_edges(edge_index, edge_vec)
        I, A, S = new_radial_tensor(
            torch.eye(3, 3, device=edge_vec.device)[None, None, :, :],
            vector_to_skewtensor(edge_vec),
            vector_to_symtensor(edge_vec),
            W1,
            W2,
            W3,
        )
    ...
    def message(self, Z_i, Z_j, I, A, S):
        Zij = self.emb2(torch.cat((Z_i, Z_j), dim=-1))[..., None, None]
        I, A, S = Zij * I, Zij * A, Zij * S
        return I, A, S
...
def update_IAS(linears, IAS):
    for i in range(len(linears)):
        IAS[i] = apply_linear(linears[i], IAS[i])
    return IAS
...
class TensorMessage(MessagePassing):
    def __init__(
        ...
        self.IASlinears = nn.ModuleList()
        self.IASlinears.append(nn.Linear(hidden_channels, hidden_channels, bias=False))
        self.IASlinears.append(nn.Linear(hidden_channels, hidden_channels, bias=False))
        self.IASlinears.append(nn.Linear(hidden_channels, hidden_channels, bias=False))
    ...
    def forward(self, X, edge_index, edge_weight, edge_attr):
        tensor_range_push("tensor_message")
        C = self.cutoff(edge_weight)
        ...
        I, A, S = decompose_tensor(X)
        I, A, S = update_IAS(self.IASlinears, [I, A, S])
        Y = I + A + S
        ...
        I, A, S = update_IAS(self.linears_tensor, [I, A, S])
        dX = I + A + S
